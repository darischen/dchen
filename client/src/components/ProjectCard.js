...
const ProjectCard = ({
  ...
  const cardStyle = categories.length > 1 ? {
    ...
    backgroundImage: `linear-gradient(#1a1a2e, #1a1a2e), linear-gradient(to bottom, ${getAccentColor(categories[0])} 0%, ${getAccentColor(categories[0])} 35%, ${getAccentColor(categories[1])} 65%, ${getAccentColor(categories[1])} 100%)`,
